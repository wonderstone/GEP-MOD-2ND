...
type GenomeHandler struct {
	Op      *Op
	Genomes []*genome.Genome // 种群
	Genome  *genome.Genome   // 存储best genome
...
func (g *GenomeHandler) Init(option ...WithOption) error {
	...
	if g.Op.NumTerminal <= 0 {
		// use default error to output
		fmt.Print("输入的终结符数量 必须大于0，请利用 SetGEPInputParams 设置")
		return nil
	}

	var functions []gene.FuncWeight
	for _, f := range g.Op.Conf.FunctionWeights {
		functions = append(
	...
	if g.Op.GenomeF != nil {
		...
		if g.Op.Record.KES != nil {
			for j := range genes {
				genes[j] = gene.New(g.Op.Record.KES[0][j], g.Op.FuncType)
			}
	...
	if g.Op.GenomeF == nil && g.Op.Perf == nil {
		fmt.Print("未指定Genome模式下的筛选函数 或者 评估函数")
	}
